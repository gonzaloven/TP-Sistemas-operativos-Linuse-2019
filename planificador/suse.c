...
int close_tid(int tid, int socket_actual){
	...
	if(thread->tid == 0)
	{
		//log_info(logger, "Recibi un close para el hilo principal del proceso %d\n", socket_actual);
		...
		log_info(logger, "Destrui el proceso %d\n", socket_actual);
		return resultado;
	}
...
int incrementar_semaforo(uint32_t tid, char* sem){
	...
	if(semaforo == NULL)
	{
		log_error(logger,"El semaforo no existe\n");
		return -1;
	}
...
void ejecucion_a_bloqueado_por_semaforo(int tid, un_socket socket, t_suse_semaforos* semaforo)
{
	...
	log_info(logger, "El thread %d se bloqueo por semaforo %s \n", thread->tid, semaforo->NAME);
}
...
pthread_t nuevo_hilo(void *(* funcion ) (void *), t_list * parametros)
{
	...
	if (thread_creacion != 0) {
		...
		log_error(logger, "Error al crear hilo para atender al proceso \n");
	} else {
		i_thread++;
	}
	return thread;
...
/////Fin funciones transicion de estados /////ᙰᒪ翄
